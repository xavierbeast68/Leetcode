...
/**
 * xavierbeast68
 * URL : https://leetcode.com/contest/biweekly-contest-100/problems/distribute-money-to-maximum-children/
...
public:
    int distMoney(int money, int children) {
        ...
        if(res == children-1 && number == 3){
            res--;
        }
        if(res > children || (res == children && number > 0)){
            res = children - 1;
        }
...
{
    /*--Let's Code--*/
    read(money);read(children);
    Solution obj;
    int ans = obj.distMoney(money, children);
    println(ans);  
}
