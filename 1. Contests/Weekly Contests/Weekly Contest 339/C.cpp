...
class Solution {
public:
    static bool customComp(const pair<int,int>& p1, const pair<int,int>& p2){
    ...
    int miceAndCheese(vector<int>& reward1, vector<int>& reward2, int k) {
        ...
        for(int i = 0; i<n; ++i){
            mp.push_back(make_pair(reward1[i], reward2[i]));
        }
        
        sort(mp.begin(), mp.end(), customComp);
        ...
        for(int i = 0; i<n; ++i){
            if((n-i) == k && k > 0){
                ...
                if(mp[i].first > mp[i].second && k > 0){
                    ans += mp[i].first;
                    k--;
                }
                else if(mp[i].first == mp[i].second ){
...
{
    /*--Let's Code--*/
    vector<int> n1{1,2,2};//{1,4,4,6,4}; //{3,3};
    vector<int> n0{2,1,2};//{6,5,3,6,1}; //{3,5};
    int k = 2;//1;
    Solution obj;
    int ans = obj.miceAndCheese(n1,n0,k);
    println(ans);  
}
